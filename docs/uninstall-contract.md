...
- **It does not remove the Worthless MCP server from your editor.** If you added Worthless to Claude Code / Cursor / Windsurf, the `worthless` entry in your MCP config (`.mcp.json`, `~/.claude.json`, or `~/.cursor/mcp.json`) is yours to remove — uninstall never edits editor configs, since they hold your other MCP servers. It *does* name the file if it finds one. Running the server via `npx worthless-mcp` also leaves a `uv`-cached Python env; `uv` is a shared tool, so uninstall doesn't touch it — reclaim the space with `uv cache clean` if you want it back.
