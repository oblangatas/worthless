...
| R-10 | `release.sh` MUST pin the wheel via `pip download --no-deps` in step 4.2 (SHA256 captured to `.release-state/<v>/wheel.sha256`), then call `gh attestation verify <pinned-wheel> --owner oblangatas --repo worthless` against the EXACT pinned file in step 4.2b. Never call `gh attestation verify` on a freshly-fetched wheel (defeats TOCTOU between PyPI lookup + verify). End-user `pip install` attestation enforcement is a separate ecosystem gap. | Mirror poisoning, name confusion, propagation-lag false-positive, TOCTOU between PyPI lookup + attestation fetch |
