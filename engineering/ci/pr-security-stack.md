...
| **Snyk** | app.snyk.io GitHub integration + README badge (server-side; **no CI job**) | None | Dashboard + Snyk email alerts |
...
The `sonarcloud` job in `tests.yml` therefore skips when `github.actor == 'dependabot[bot]'`. Human-authored PRs are unchanged. Meaningful gates on dependency bumps remain: pytest matrix, Semgrep, CodeQL, Bandit, coverage floors, and Grype on the built image (`docker-security.yml`, blocking at medium+ with a fix available).
