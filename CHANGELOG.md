...
### Changed
- **The MCP server runs on the mcp 2.x SDK** (WOR-929). The `mcp` extra moves from `mcp>=1.0,<2` to `mcp>=2.1,<3`, and the server imports `MCPServer` from `mcp.server.mcpserver` (2.x renamed `FastMCP` and removed the old module). mcp 2.x hides the text of any tool exception that is not its own `ToolError`, which would have turned "Worthless is not initialized. Run `worthless lock` first." into a bare "Error executing tool worthless_spend" for the agent. `WorthlessError` messages, which are written for users, are now passed through as `ToolError`; any other crash stays hidden from the client. A new stdio test proves this through a real MCP client: tool schemas, a success result, and an error result carrying the message. The dependency guard now rejects any range that admits 1.x (no `mcp.server.mcpserver`) or 3.x. Upgrading with plain `pip install -U worthless` keeps the old mcp 1.x, and `worthless mcp` used to die there with an opaque `WRTLS-199`; it now says `WRTLS-100 … Reinstall with the extra: pip install -U 'worthless[mcp]'`. The server also reports its own version in the MCP handshake again (2.x reports an empty string unless told).
  ...
  **What this does NOT do.**
  ...
  - Other failures, such as a permission error while scanning or a locked database, reach the agent with no reason at all (WOR-932).
