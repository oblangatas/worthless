...
- **The live kernel proof is not a committed test** (`worthless-snjs`). The A/B/C above was run on an ad-hoc harness that is not in the repository and does not run in CI, so nothing currently prevents this from regressing silently. `tests/test_cli_dumpable.py` covers the readback, the strict/warn split and the ordering guard, but not the piped-`core_pattern` bypass itself.
