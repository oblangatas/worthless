...
Full install options (Docker, MCP for AI editors — Claude Code verified, Cursor/Windsurf unverified, GitHub Actions, the verified-install flow, kill-switch runbook): **[docs.wless.io](https://docs.wless.io)**
...
Everything lives at **[docs.wless.io](https://docs.wless.io)**, install guides, the security model, wire protocol, recovery runbook, the verified-install flow, and the agent skill file (Claude Code verified; Cursor/Windsurf unverified).
...
Restart Claude Code and the MCP tools appear immediately — verified on Claude Code; Cursor/Windsurf read MCP config from their own paths and are unverified. On first run, `worthless-mcp` bootstraps `uv` and installs the Python package automatically. Install time < 30 s.
